function userList() {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        var p = roundNumber(getUserPoint(i+1, 100, entry.percentToQualify, "144hz") * 1.25, 3);
        if (checkVerify == true) {
            for (var b = 0 ; b < user_data.length ; b++) {
...
function getUserData(user) {
    ...
    for (var i = 0 ; i < user_data[user].verified.length ; i++) {
        ...
        clears++;
        progresses = progresses + '<li>' + list[rank].name + ' Verified </strong>(#'+(rank+1)+' / UP: '+roundNumber(getUserPoint(rank+1, 100, list[rank].percentToQualify, "144hz")*1.25, 3)+')<strong></a></li>'
    }
